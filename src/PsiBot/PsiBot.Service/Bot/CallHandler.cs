...
namespace PsiBot.Services.Bot
{
    ...
    public class CallHandler : HeartbeatHandler
    {
        ...
        {
            Console.WriteLine($"[MEDIA] Initializing CallHandler for call {statefulCall.Id}");
            this.botConfiguration = botConfiguration;

            try
            {
                Console.WriteLine($"[MEDIA] Creating pipeline with diagnostics enabled");
                this.pipeline = Pipeline.Create(enableDiagnostics: true);
                this.teamsBot = CreateTeamsBot(this.pipeline);

                Console.WriteLine($"[MEDIA] Bot configuration settings:");
                Console.WriteLine($"[MEDIA] MediaServiceFQDN: {botConfiguration.MediaServiceFQDN}");
                Console.WriteLine($"[MEDIA] InstancePublicPort: {botConfiguration.InstancePublicPort}");
                Console.WriteLine($"[MEDIA] UseLocalDevSettings: {botConfiguration.UseLocalDevSettings}");

                PsiExporter exporter = null;

                if (!string.IsNullOrEmpty(botConfiguration.PsiStoreDirectory))
                {
                    Console.WriteLine($"[MEDIA] Setting up PsiStore in directory: {botConfiguration.PsiStoreDirectory}");
                ...
                Console.WriteLine("[MEDIA] Getting local media session");
                var mediaSession = this.Call.GetLocalMediaSession();
                Console.WriteLine($"[MEDIA] Media session obtained. AudioSocket: {mediaSession.AudioSocket != null}");

                // Log video socket information
                var videoSockets = mediaSession.VideoSockets;
                Console.WriteLine($"[MEDIA] Number of video sockets: {videoSockets.Count}");
                foreach (var socket in videoSockets)
                {
                    Console.WriteLine($"[MEDIA] Video socket ID: {socket.SocketId}");
                    this.availableSocketIds.Add((uint)socket.SocketId);
                }

                // Subscribe to the participants updates
                Console.WriteLine("[MEDIA] Setting up participant updates subscription");
                this.Call.Participants.OnUpdated += this.ParticipantsOnUpdated;

                Console.WriteLine("[MEDIA] Setting up dominant speaker detection");
                ...
                {
                    Console.WriteLine($"[MEDIA] Call state updated to: {call.Resource.State}");
                    if (waitingToShare && call.Resource.State == CallState.Established && this.teamsBot.EnableScreenSharing)
                    {
                        Console.WriteLine("[MEDIA] Call established, enabling screen sharing");
                        try
                        {
                            this.Call.ChangeScreenSharingRoleAsync(ScreenSharingRole.Sharer).Wait();
                            waitingToShare = false;
                            Console.WriteLine("[MEDIA] Screen sharing role changed successfully");
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"[MEDIA ERROR] Failed to change screen sharing role: {ex.Message}");
                        }
                    }
                };

                Console.WriteLine("[MEDIA] Creating BotMediaStream");
                try
                {
                    ...
                    Console.WriteLine("[MEDIA] BotMediaStream created successfully");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[MEDIA ERROR] Failed to create BotMediaStream: {ex.Message}");
                    if (ex.InnerException != null)
                    {
                        Console.WriteLine($"[MEDIA ERROR] Inner exception: {ex.InnerException.Message}");
                    }
                    throw;
                }
                ...
                {
                    Console.WriteLine($"[MEDIA ERROR] Pipeline exception: {ex.Exception.Message}");
                    if (ex.Exception.InnerException != null)
                    {
                        Console.WriteLine($"[MEDIA ERROR] Pipeline inner exception: {ex.Exception.InnerException.Message}");
                    }
                };

                Console.WriteLine("[MEDIA] Starting pipeline");
                this.pipeline.RunAsync();
                Console.WriteLine("[MEDIA] Pipeline started successfully");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MEDIA ERROR] CallHandler initialization failed: {ex.Message}");
                if (ex.InnerException != null)
                {
                    Console.WriteLine($"[MEDIA ERROR] Inner exception: {ex.InnerException.Message}");
                }
